...
void main() {
  ...
  SystemChrome.setEnabledSystemUIMode(SystemUiMode.immersiveSticky); // <-- Add this line
...
class _DroneControlScreenState extends State<DroneControlScreen> with SingleTickerProviderStateMixin {
  ...
  try {
    ...
    final statvfsExt = extDir != null ? await FileStat.stat(extDir.path) : null;
    ...
    if (totalSpace != null && freeSpace != null) {
      ...
      setState(() {
        _storage = '${(used / 1024 / 1024 / 1024).toStringAsFixed(2)} / ${(totalSpace / 1024 / 1024 / 1024).toStringAsFixed(2)} GB';
      });
    } else {
...
Future<int?> _getTotalSpace(String path) async {
  ...
  Future<void> _updateWiFi() async {
    ...
    setState(() => _wifi = connectivity.contains(ConnectivityResult.wifi) ? 'Connected' : 'Disconnected');
  }
                  ...
                  Positioned.fill(
                    ...
                    child: CustomPaint(
                      painter: _HeightScalePainter(
                        tickCount: 11, // Odd number for symmetry, covers -25 to +25 if majorTickEvery=5
                        majorTickEvery: 5,
                  ...
                  AnimatedBuilder(
                    ...
                    builder: (context, child) {
                      return Container(
                        ...
                        height: 15,
                        decoration: BoxDecoration(
                          color: Colors.red.withAlpha((_animationController.value* 255).toInt()),
                          shape: BoxShape.circle,
                          boxShadow: [BoxShadow(color: Colors.red.withAlpha( 130), blurRadius: 5)],
                        ),
                      );
                    },
                  ),
          ...
          Positioned(
            ...
            child: Joystick(
              ...
              onChanged: (offset) {
                ...
                _rcTimer ??= Timer.periodic(const Duration(milliseconds: 100), (_) => _sendRCCommand());
              },
              onReleased: () {
                setState(() => _leftJoystick = Offset.zero);
                _rcTimer?.cancel();
                _rcTimer = null;
                _sendCommand('rc', {'left_right': 0, 'forward_backward': 0, 'up_down': 0, 'yaw': 0});
              },
            ),
          ),
          Positioned(
            ...
            child: Column(
              ...
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Icon(Icons.flight_takeoff, color: Colors.indigo.shade700, size: 20),
                    ...
                    children: [
                      _FlightModeOption(
                        ...
                        onTap: _isCommanding
                            ? null
                            : () {
                                setState(() => _showFlightModes = false);
                                _sendCommand('follow');
                              },
                      ),
    ...
    return GestureDetector(
      ...
      if (widget.isLeft) ...[
        ...
        Positioned(
          top: 10,
          left: 60,
          child: Icon(Icons.arrow_upward, color: Colors.blueGrey.shade700, size: 28),
        ),
        // Down
        Positioned(
          bottom: 10,
          left: 60,
          child: Icon(Icons.arrow_downward, color: Colors.blueGrey.shade700, size: 28),
        ),
        // Rotate Left
        Positioned(
          left: 10,
          top: 60,
          child: Icon(Icons.rotate_left, color: Colors.blueGrey.shade700, size: 28),
        ),
        // Rotate Right
        Positioned(
          right: 10,
          top: 60,
          child: Icon(Icons.rotate_right, color: Colors.blueGrey.shade700, size: 28),
        ),
      ] else ...[
        // Up Arrow
        Positioned(
        ...
        Positioned(
          left: 10,
          top: 60,
          child: Icon(Icons.arrow_back, color: Colors.blueGrey.shade700, size: 28),
        ),
        // Right Arrow
        Positioned(
          right: 10,
          top: 60,
          child: Icon(Icons.arrow_forward, color: Colors.blueGrey.shade700, size: 28),
        ),
      ],
            // Knob
            Transform.translate(
